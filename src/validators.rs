...
use crate::messages::{
    ...
    DESCRIPTION_NO_LEADING_SPACE_ERROR, HEADER_LENGTH_ERROR, INCORRECT_FORMAT_ERROR,
    SCOPE_EMPTY_ERROR, SCOPE_WHITESPACE_ERROR, SPACE_AFTER_COMMIT_TYPE_ERROR,
    SPACE_AFTER_SCOPE_ERROR, commit_type_invalid_error,
};
use regex::Regex;
...
pub fn validate_header_length(message: &str) -> Option<String> {
    let header = message.lines().next().unwrap_or("");
    if header.len() > COMMIT_HEADER_MAX_LENGTH {
        Some(HEADER_LENGTH_ERROR.to_string())
    } else {
        None
    }
}
...
pub fn run_validators(message: &str, skip_detail: bool) -> (bool, Vec<String>) {
    if skip_detail {
        if let Some(error) = validate_header_length(message) {
...
mod tests {
    ...
    fn header_length_rejects_long_header() {
        ...
        assert_eq!(
            validate_header_length(&message),
            Some(HEADER_LENGTH_ERROR.to_string())
        );
    }
    ...
    fn skip_detail_returns_only_header_length_error() {
        ...
        let (success, errors) = run_validators(&message, true);
        assert!(!success);
        assert_eq!(errors, vec![HEADER_LENGTH_ERROR.to_string()]);
    }

    #[test]
    fn skip_detail_returns_only_incorrect_format_error() {
        let (success, errors) = run_validators("Test invalid commit message", true);
        assert!(!success);
